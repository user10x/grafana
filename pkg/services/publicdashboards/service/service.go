...
import (
	...
	infra "github.com/grafana/grafana/pkg/api"
...
func ProvidePublicDashboardService(
	...
	hs *infra.HTTPServer,
	qs *query.Service,
) (*PublicDashboardServiceImpl, error) {
	// build the service
	s := &PublicDashboardServiceImpl{
		log:   log.New("publicdashboards"),
		cfg:   cfg,
		store: database.NewPublicDashboardStore(sqlStore),
	}

	// attach api if feature is enabled
	if features.IsEnabled(featuremgmt.FlagPublicDashboards) {
		api := api.NewPublicDashboardApi(s, rr, ac, hs, qs)

		api.RegisterAPIEndpoints(features)
	}
